...
function register_user($conn)
{
    ...
    if (count($eroor_Array) <= 0) {
        $query = "INSERT INTO `users`(`id`,`employee_id`, `username`, `password`, `image`)
        VALUES('$new_id','$employee_id', '$username',  MD5('$password'),  '$save_name')";
...
function read_all_employe_name($conn)
{
    $data = array();
    $array_data = array();
    $query = "SELECT  e.employee_id, concat(e.fristname, ' ', e.lastname) as employee_name from employee e";
    $result = $conn->query($query);
...
function get_user_list($conn)
{
    $data = array();
    $array_data = array();
    $query = "SELECT u.id,concat(e.fristname, ' ', e.lastname) as employe_name, u.username,u.image,u.status,u.date from users u JOIN employee e on u.employee_id=e.employee_id";
    $result = $conn->query($query);
...
function update_user($conn)
{
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        if (count($eroor_Array) <= 0) {
            $query = "UPDATE users set users.employee_id= '$employee_id',  users.username= '$username', 
